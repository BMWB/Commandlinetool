...
//import Alamofire
extension String{
    public func appPath(_ value:String) -> String {
        if self.hasSuffix("/") {
            return self + value
        }
        return self + "/" + value
...
public struct IpaTool {
    
    public struct Output {
        public var pipe:Pipe
        public var readData:String
        init(pipe:Pipe) {
            self.pipe = pipe
    ...
    public init(projectPath:String,
        ...
        self.scheme = schemeKey ?? (allFiles[0].components(separatedBy: ".").first ?? "")
...
extension IpaTool{
    ...
    public func clean()->Output{
        let arguments = ["clean",
                         ...
                         "-sdk iphoneos build CODE_SIGN_IDENTITY = iPhone Distribution: Beijing China Product Bo Farce Network Technology Co., Ltd. (WULLLP6Z85) PROVISIONING_PROFILE = 4c49d399-03ac-4296-9c89-d0d6432d8611 PROVISIONING_PROFILE_SPECIFIER = DistShopzp ",
                         "-archivePath",
                         xcarchive,
                         "-quiet"
                        ]
    ...
    public func exportArchive()->Output{
